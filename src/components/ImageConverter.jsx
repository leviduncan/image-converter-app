  ...
  return (
    <div className={`${darkMode ? 'bg-dark text-light' : 'bg-light text-dark'} min-vh-100 pt-4`}>
      <main className="container">
        <div className="hero rounded-4 mb-4">
        <div className="d-flex justify-content-between align-items-center mb-4">
          <h2 className="text-center mb-4 text-light">Image Converter</h2>
          <button className="btn btn-warning mb-3" onClick={() => setDarkMode(!darkMode)}>
            Toggle {darkMode ? 'Light' : 'Dark'} Mode
          </button>
        </div>
        <div {...getRootProps()} className="border p-5 text-center mb-3 rounded-4" style={{ cursor: "pointer" }}>
          <input {...getInputProps()} />
        ...
        <div className="mb-3">
          ...
          <div className="btn-group w-100" role="group">
            {['jpeg', 'png', 'webp'].map(f => (
              <button key={f} className={`btn ${format === f ? 'btn-primary' : 'btn-outline-primary'}`} onClick={() => setFormat(f)}>
                {f.toUpperCase()}
              </button>
            ))}
          </div>
        ...
        <button className="btn btn-primary w-100 mb-3" onClick={convertImages} disabled={selectedFiles.length === 0}>
          Convert Selected
        </button>
